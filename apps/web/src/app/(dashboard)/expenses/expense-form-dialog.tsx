...
interface FormState { expenseNumber: string; category: string; vendorName: string; vendorId: string; amount: string; currencyCode: string; status: string; description: string; expenseDate: string; }
import { getDefaultCurrency } from '@/stores/auth-store';
const empty: FormState = { expenseNumber: '', category: '', vendorName: '', vendorId: '', amount: '0', currencyCode: getDefaultCurrency(), status: 'PENDING', description: '', expenseDate: '' };
...
export function ExpenseFormDialog({ open, onOpenChange, expense, onSaved }: Props) {
  ...
  useEffect(() => { if (open) { setError(''); setForm(expense ? { expenseNumber: expense.expenseNumber, category: expense.category ?? '', vendorName: expense.vendorName ?? '', vendorId: (expense as any).vendorId ?? '', amount: String(expense.amount), currencyCode: expense.currencyCode, status: expense.status, description: expense.description ?? '', expenseDate: expense.expenseDate?.split('T')[0] ?? '' } : empty); } }, [open, expense]);
  const set = (k: keyof FormState, v: string) => setForm((f) => ({ ...f, [k]: v }));
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!activeTenant) return;
    setSaving(true); setError('');
    const payload = { ...(form.expenseNumber.trim() ? { expenseNumber: form.expenseNumber.trim() } : {}), category: form.category.trim() || undefined, vendorName: form.vendorName.trim() || undefined, vendorId: form.vendorId || undefined, amount: Number(form.amount) || 0, currencyCode: form.currencyCode, status: form.status, description: form.description.trim() || undefined, expenseDate: form.expenseDate || undefined };
    try {
