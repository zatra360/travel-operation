...
export default function PerformancePage() {
  ...
  return (<div className="space-y-6"><PageHeader title="Performance Reviews" subtitle="Conduct and review employee performance evaluations" actions={<Button size="sm" onClick={openCreate}><Plus className="h-4 w-4 mr-2" />New Review</Button>} />
    <Card><CardHeader><CardTitle>All Reviews</CardTitle></CardHeader><CardContent>{loading ? <TableSkeleton /> : error ? <div className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">{error}</div> : items.length === 0 ? <div className="py-10 text-center"><p className="text-muted-foreground">No reviews.</p><Button size="sm" variant="outline" className="mt-3" onClick={openCreate}><Plus className="h-4 w-4 mr-2" />Create review</Button></div> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="pb-3 font-medium">Employee</th><th className="pb-3 font-medium">Period</th><th className="pb-3 font-medium">Rating</th><th className="pb-3 font-medium">Status</th><th className="pb-3 font-medium">Created</th><th className="pb-3 font-medium text-right">Actions</th></tr></thead><tbody>{items.map((r) => (<tr key={r.id} className="border-b last:border-0"><td className="py-3 font-medium">{r.employee?.firstName} {r.employee?.lastName}</td><td className="py-3">{r.period}</td><td className="py-3">{r.rating ? `${r.rating}/5` : '--'}</td><td className="py-3"><Badge variant={reviewStatusVariant[r.status] || 'secondary'}>{r.status}</Badge></td><td className="py-3 text-muted-foreground">{formatDate(r.createdAt)}</td><td className="py-3"><div className="flex items-center justify-end gap-1"><Button variant="ghost" size="icon" onClick={() => openEdit(r)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => setDeleting(r)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></td></tr>))}</tbody></table></div>}</CardContent></Card>
