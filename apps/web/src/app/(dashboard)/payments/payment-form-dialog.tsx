...
const empty: FormState = { bookingId: '', invoiceId: '', amount: '0', currencyCode: getDefaultCurrency(), paymentMethod: '', status: 'PENDING', reference: '', notes: '', receivedAt: '' };
...
export function PaymentFormDialog({ open, onOpenChange, payment, onSaved }: Props) {
  ...
  return (<Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-xl"><DialogHeader><DialogTitle>{isEdit ? 'Edit Payment' : 'New Payment'}</DialogTitle><DialogDescription>{isEdit ? 'Update payment details.' : 'Record a new payment.'}</DialogDescription></DialogHeader>
    <form onSubmit={handleSubmit} className="space-y-4">{error && <div className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">{error}</div>}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="paymentMethod">Method</Label><Select value={form.paymentMethod} onValueChange={(v) => set('paymentMethod', v)}><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger><SelectContent><SelectItem value="CASH">Cash</SelectItem><SelectItem value="BANK_TRANSFER">Bank Transfer</SelectItem><SelectItem value="CARD">Card</SelectItem><SelectItem value="MOBILE">Mobile</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Bank Account</Label><BankAccountSelect value={(form as any).bankAccountId} onChange={(v) => set('bankAccountId' as any, v)} /></div></div>
