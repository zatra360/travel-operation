...
export default function NotificationsPage() {
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><h2 className="text-2xl font-bold">Notifications</h2>{unread > 0 && <Button size="sm" variant="outline" onClick={markAll}>Mark all read ({unread})</Button>}</div>
    <Card><CardHeader><CardTitle>Recent</CardTitle></CardHeader><CardContent>{loading ? <p className="text-muted-foreground">Loading...</p> : items.length === 0 ? <p className="text-muted-foreground">No notifications.</p> : <div className="space-y-2">{items.map((n) => (<div key={n.id} className={`flex items-start justify-between gap-4 rounded-md border p-3 ${n.isRead ? 'opacity-60' : ''}`}><div className="min-w-0"><p className="font-medium text-sm">{n.title}</p>{n.body && <p className="text-xs text-muted-foreground mt-1">{n.body}</p>}<p className="text-xs text-muted-foreground mt-1">{formatDate(n.createdAt)}</p></div><div className="flex items-center gap-2 shrink-0"><Badge variant={n.isRead ? 'secondary' : 'default'}>{n.type}</Badge>{!n.isRead && <Button variant="ghost" size="sm" onClick={() => markRead(n.id)}>Read</Button>}</div></div>))}</div>}</CardContent></Card></div>);
}
