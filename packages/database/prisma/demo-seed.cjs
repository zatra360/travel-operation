...
async function main() {
  ...
  const q1 = await prisma.quotation.create({ data: { tenantId: tenant.id, branchId: ho.id, quoteNumber: 'QTE-2607-1001', status: 'SENT', title: 'Umrah Package', clientId: c1.id, assignedToId: sales1.id, currencyCode: 'BDT', subtotal: 333333, taxTotal: 16667, grandTotal: 350000, validUntil: new Date('2026-08-15'), createdById: sales1.id, sentAt: new Date() } });
  await prisma.quotationLineItem.create({ data: { tenantId: tenant.id, quotationId: q1.id, serviceType: 'UMRAH', title: 'Umrah Package', description: '14-day package', quantity: 1, unitPrice: 333333, lineTotal: 333333, sortOrder: 1 } });

  const q2 = await prisma.quotation.create({ data: { tenantId: tenant.id, branchId: dhk.id, quoteNumber: 'QTE-2607-1002', status: 'ACCEPTED', title: 'Dubai Visa', clientId: c2.id, assignedToId: sales2.id, currencyCode: 'USD', subtotal: 1190, taxTotal: 60, grandTotal: 1250, validUntil: new Date('2026-09-01'), createdById: sales2.id, sentAt: new Date(), acceptedAt: new Date() } });
  await prisma.quotationLineItem.create({ data: { tenantId: tenant.id, quotationId: q2.id, serviceType: 'VISA', title: 'Dubai Visa', description: '30-day tourist visa x2', quantity: 2, unitPrice: 595, lineTotal: 1190, sortOrder: 1 } });
  console.log('  Quotations: 2 with line items');

  const b1 = await prisma.booking.create({ data: { tenantId: tenant.id, branchId: ho.id, bookingRef: 'BKG-2607-1001', pnrLocator: 'ABC123', status: 'CONFIRMED', clientId: c1.id, quotationId: q1.id, assignedToId: ticketing.id, travelStart: new Date('2026-07-10'), travelEnd: new Date('2026-07-24'), createdById: ticketing.id } });
  await prisma.bookingPassenger.create({ data: { tenantId: tenant.id, bookingId: b1.id, passengerType: 'ADULT', firstName: 'Rahim', lastName: 'Uddin' } });
  ...
  const b2 = await prisma.booking.create({ data: { tenantId: tenant.id, branchId: dhk.id, bookingRef: 'BKG-2607-1002', pnrLocator: 'XYZ789', status: 'TICKETED', clientId: c2.id, quotationId: q2.id, assignedToId: ticketing.id, travelStart: new Date('2026-08-05'), travelEnd: new Date('2026-08-12'), createdById: ticketing.id } });
  ...
  const inv1 = await prisma.invoice.create({ data: { tenantId: tenant.id, branchId: ho.id, invoiceNumber: 'INV-2607-1001', status: 'PAID', clientId: c1.id, bookingId: b1.id, currencyCode: 'BDT', subtotal: 333333, taxAmount: 16667, totalAmount: 350000, paidAmount: 350000, dueAmount: 0, createdById: finance.id, issuedAt: new Date() } });
  ...
  const inv2 = await prisma.invoice.create({ data: { tenantId: tenant.id, branchId: dhk.id, invoiceNumber: 'INV-2607-1002', status: 'PARTIALLY_PAID', clientId: c2.id, bookingId: b2.id, currencyCode: 'USD', subtotal: 1190, taxAmount: 60, totalAmount: 1250, paidAmount: 500, dueAmount: 750, createdById: finance.id, issuedAt: new Date(), dueAt: new Date('2026-08-15') } });
  ...
  const pay1 = await prisma.payment.create({ data: { tenantId: tenant.id, branchId: ho.id, invoiceId: inv1.id, bookingId: b1.id, clientId: c1.id, amount: 350000, currencyCode: 'BDT', paymentMethod: 'BANK_TRANSFER', status: 'RECEIVED', reference: 'BANK-001', createdById: finance.id, receivedAt: new Date() } });
  ...
  const pay2 = await prisma.payment.create({ data: { tenantId: tenant.id, branchId: dhk.id, invoiceId: inv2.id, bookingId: b2.id, clientId: c2.id, amount: 500, currencyCode: 'USD', paymentMethod: 'CASH', status: 'RECEIVED', createdById: finance.id, receivedAt: new Date() } });
  ...
  await prisma.ticket.create({ data: { tenantId: tenant.id, branchId: dhk.id, bookingId: b2.id, ticketNumber: 'TKT-2607-1001', passengerName: 'Ayesha Siddique', status: 'ISSUED', issuedAt: new Date(), createdById: ticketing.id } });
  await prisma.ticket.create({ data: { tenantId: tenant.id, branchId: dhk.id, bookingId: b2.id, ticketNumber: 'TKT-2607-1002', passengerName: 'Aariz Siddique', status: 'PENDING', createdById: ticketing.id } });
